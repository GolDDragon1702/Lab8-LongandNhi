...
from rag_answer import rag_answer, generator
...
BASELINE_CONFIG = {
    "retrieval_mode": "filter",   # Hoặc "dense" nếu chỉ đổi rerank
    "top_k_search": 10,
    "top_k_select": 3,
...
def score_faithfulness(
    ...
    if not answer or not chunks_used:
        return {"score": 1, "notes": "No answer or no chunks retrieved"}

    context_str = "\n\n".join(
        f"[{i+1}] {c.get('text', '')[:300]}" for i, c in enumerate(chunks_used)
    )
...
1 = Answer is mostly hallucinated, not grounded in context
...
3 = Related but doesn't fully address the core question
...
Respond ONLY with valid JSON, no markdown, no explanation outside JSON:
{{"score": <int 1-5>, "missing_points": ["<point>", "..."], "reason": "<one sentence>"}}"""
